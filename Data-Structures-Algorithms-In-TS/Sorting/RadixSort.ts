...
function countSort(arr: number[], n: number, dividend: number): void {
    ...
    for (let i = 0; i < n; i++) {
        count[Math.floor(temp[i] / dividend) % 10]++;
    }
    ...
    for (let i = n - 1; i >= 0; i--) {
        arr[count[Math.floor(temp[i] / dividend) % 10] - 1] = temp[i];
        count[Math.floor(temp[i] / dividend) % 10]--;
    }
}
...
function radixSort(arr: number[]): void {
    ...
    for (let div = 1; Math.floor(m / div) > 0; div *= 10) {
        countSort(arr, n, div);
    }
}

// Testing code.
let array: number[] = [100, 49, 65, 91, 702, 29, 4, 55];
radixSort(array);
