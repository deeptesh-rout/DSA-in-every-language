...
function closestPairDC(arr: number[][]): number {
    const n = arr.length;
    const p: Point[] = Array(n).fill(null);
    for (let i = 0; i < n; i++) {
        p[i] = new Point(arr[i][0], arr[i][1]);
    }
    // Sort according to x axis.
    p.sort((s1, s2) => s1.x - s2.x);
    let q = [...p];
    // Sort according to y axis.
    q.sort((s1, s2) => s1.y - s2.y);
    return closestPairUtil(p, 0, n - 1, q, n);
}

function closestPairUtil(p: Point[], start: number, stop: number, q: Point[], n: number): number {
    if (stop - start < 1) {
        return 999999;
    }
    if (stop - start === 1) {
    ...
    let mid = parseInt(((start + stop) / 2).toString());
    ...
    let strip: Point[] = Array(n).fill(null);
    let j = 0;
    for (let i = 0; i < n; i++) {
...
// Testing code.
const arr: number[][] = [[648, 896], [269, 879], [250, 922], [453, 347], [213, 17]];
